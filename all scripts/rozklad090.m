...
ylim([-3 45]);
xlim([-5 40]);
title('Rozk³ad b³edu pozycji dla znaku na trawersie l.b.'); 
xlabel('X [m]')
ylabel('Y [m]')
ax = gca;
ax.FontSize = 15; 
...
filename=['C:\Users\Admin\Dysk Google\AKADEMIA\programy\MATLAB\skrypty\doktorat\symulacja_duza\wykresy\rozklad090.jpg'];


xa = [xz_0-3 xz_0]; % X-Coordinates in data space
ya = [yz_0-1 yz_0]; % Y-Coordinates in data space
[xaf,yaf] = ds2nfu(xa,ya); % Convert to normalized figure units
a=annotation('textarrow',xaf,yaf,'String','X_z ')
set(a,'FontSize',15,'FontAngle','italic','FontName','Times','HeadStyle','none');

xa = [poz_x1+4 poz_x1]; % X-Coordinates in data space
ya = [poz_y1+10 poz_y1]; % Y-Coordinates in data space
[xaf,yaf] = ds2nfu(xa,ya); % Convert to normalized figure units
a=annotation('textarrow',xaf,yaf,'String','X_1 ')
set(a,'FontSize',15,'FontAngle','italic','FontName','Times','HeadStyle','none');

xa = [0+2 0]; % X-Coordinates in data space
ya = [0+10 0]; % Y-Coordinates in data space
[xaf,yaf] = ds2nfu(xa,ya); % Convert to normalized figure units
...
legend('pojedyncza wyznaczona pozycja znaku X_z','elipsa b³êdu pozycji znaku dla p=0,95%','wyznaczona pozycja pojazdu X_1','elipsa b³êdu pozycji pojazdu p=0,95%')
diff_x=max_x1-min_x1;
diff_y=max_y1-min_y1;
proporcja=diff_x/diff_y;
skala1=8.5;
...
diff_x1=diff_x*skala1;
diff_y1=diff_y*skala1;
MagInset(h1, -1, [min_x0 max_x0 min_y0 max_y0], [10 10+diff_x1 15 15+diff_y1], {'NW','NW';'SE','SE'});
