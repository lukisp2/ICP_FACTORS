...
ylim([-7 38]);
xlim([-1 44]);
title('Rozk³ad b³edu pozycji dla znaku na k¹cie kursowym 060^o l.b.'); 
xlabel('X [m]')
ylabel('Y [m]')
ax = gca;
ax.FontSize = 15; 
...
filename=['C:\Users\Admin\Dysk Google\AKADEMIA\programy\MATLAB\skrypty\doktorat\symulacja_duza\wykresy\rozklad060.jpg'];


xa = [xz_0+5 xz_0]; % X-Coordinates in data space
...
ya = [poz_y1+6 poz_y1]; % Y-Coordinates in data space
[xaf,yaf] = ds2nfu(xa,ya); % Convert to normalized figure units
a=annotation('textarrow',xaf,yaf,'String','X_1 ')
set(a,'FontSize',15,'FontAngle','italic','FontName','Times','HeadStyle','none');

xa = [0+2 0]; % X-Coordinates in data space
ya = [0-3 0]; % Y-Coordinates in data space
[xaf,yaf] = ds2nfu(xa,ya); % Convert to normalized figure units
...
lgd=legend('wyznaczona pozycja znaku X_z','elipsa b³êdu pozycji znaku dla p=0,95%','wyznaczona pozycja pojazdu X_1','elipsa b³êdu pozycji pojazdu p=0,95%','location','southeast')
lgd.NumColumns =2;

MagInset(h1, -1, [min_x1 max_x1 min_y1 max_y1], [8 8+diff_x1 5 5+diff_y1], {'NW','NW';'SE','SE'});
