...
ylim([-12.5 32.5]);
xlim([0 45]);
title('Rozk³ad b³edu pozycji dla znaku na k¹cie kursowym 030^o l.b.'); 
xlabel('X [m]')
ylabel('Y [m]')
ax = gca;
ax.FontSize = 15; 
...
filename=['C:\Users\Admin\Dysk Google\AKADEMIA\programy\MATLAB\skrypty\doktorat\symulacja_duza\wykresy\rozklad030.jpg'];


xa = [xz_0+5.5 xz_0]; % X-Coordinates in data space
ya = [yz_0-7 yz_0]; % Y-Coordinates in data space
...
ya = [poz_y1-5 poz_y1]; % Y-Coordinates in data space
[xaf,yaf] = ds2nfu(xa,ya); % Convert to normalized figure units
a=annotation('textarrow',xaf,yaf,'String','X_1 ')
set(a,'FontSize',15,'FontAngle','italic','FontName','Times','HeadStyle','none');

xa = [0+7 0]; % X-Coordinates in data space
ya = [0-5 0]; % Y-Coordinates in data space
[xaf,yaf] = ds2nfu(xa,ya); % Convert to normalized figure units
...
MagInset(h1, -1, [min_x1 max_x1 min_y1 max_y1], [10 10+diff_x1 3 3+diff_y1], {'NW','NW';'SE','SE'});
...
MagInset(h1, -1, [min_x0 max_x0 min_y0 max_y0], [25 25+diff_x1 3 3+diff_y1], {'NW','NW';'SE','SE'});
